import type { BranchDiff, GraphBranch, NodeDiff, EdgeDiff } from "@abhinav2203/codeflow-core/schema";
...
export const searchBranches = async ({
  ...
  query,
  limit = 5
}: SearchBranchesOptions): Promise<BranchSearchResult[]> => {
  ...
  try {
    const result = await codeRag.query(
      `Search branches for project ${projectName}: ${query}`,
      { depth: 2 }
    );
 ...
 */
export const formatStructuralDiff = (
  diff: BranchDiff,
  focusOn: "nodes" | "edges" | "all" = "all"
): string => {
  const lines: string[] = [];
  ...
  lines.push("### Summary");
  lines.push(
    `- Added: ${diff.addedNodes} node(s), ${diff.addedEdges} edge(s)`
  );
  lines.push(
    `- Removed: ${diff.removedNodes} node(s), ${diff.removedEdges} edge(s)`
  );
  lines.push(
    `- Modified: ${diff.modifiedNodes} node(s)`
  );
  lines.push("");
...
export const explainBranchDiff = async ({
  ...
  compareBranch,
  focusOn = "all"
}: ExplainBranchDiffOptions): Promise<string> => {
  const codeRag = getCodeRagInstance();
