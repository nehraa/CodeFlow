...
// ReasoningCheckpoint type - mirrors the store's checkpoint shape
...
vi.mock("@abhinav2203/codeflow-store/checkpoint", () => ({
  recoverRun: vi.fn<
    (runId: string, projectName: string) => Promise<ReasoningCheckpoint[]>
  >()
}));
...
vi.mock("@abhinav2203/codeflow-store/reasoning", () => ({
  ...
  loadReasoningForProject: vi.fn<
    (projectName: string) => Promise<
      Array<{ runId: string; projectName: string; checkpoints: ReasoningCheckpoint[] }>
    >
  >()
}));
...
const makeCheckpoint = (overrides: Partial<ReasoningCheckpoint> = {}): ReasoningCheckpoint => ({
  runId: "run-1",
