...
const compactList = (items, limit) => items
    .map((item) => item.trim())
    .filter(Boolean)
    .slice(0, limit);
const lineRangeLabel = (node) => `${node.filePath}:${node.startLine}-${node.endLine}`;
const createExcerpt = (node) => {
...
const formatRetrievedNode = (node, label) => {
    const parts = [
        `${label}: ${node.name}`,
        `Kind: ${node.kind}`,
...
export const buildAgentRetrievalQuery = ({ node, relatedNodes, instruction }) => {
    ...
    const neighbors = compactList(relatedNodes
        .filter((candidate) => candidate.id !== node.id)
        .map((candidate) => candidate.name), 4);
    const parts = [
...
export const formatAgentRetrievalPrompt = (result) => {
    const sections = [
...
export const formatAgentRetrievalNote = (context) => {
...
export async function resolveAgentRetrievalContext({ node, relatedNodes, instruction, retrievalQuery, retrievalDepth, allowAutoQuery = true }) {
