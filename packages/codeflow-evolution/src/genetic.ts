...
export const TOURNAMENT_PROVENANCE = "heuristic" as const;
export const TOURNAMENT_MATURITY = "experimental" as const;
...
// ── Graph Metrics (inlined from @/lib/blueprint/metrics) ───────────────────────
...
const computeGraphMetrics = (graph: GraphMetricsInput): GraphMetrics => {
  ...
  const totalMethods = nodes.reduce(
    (sum, n) => sum + (n.contract.methods?.length ?? 0),
    0,
  );
  ...
  const avgResponsibilitiesPerNode =
    nodeCount === 0 ? 0 : totalResponsibilities / nodeCount;

  const nodeIds = nodes.map((n) => n.id);
  const connectedComponents =
    nodeCount === 0 ? 0 : computeConnectedComponents(nodeIds, edges);
...
export const benchmarkVariant = (
  ...
  const scalabilityBase =
    style === "serverless"
      ...
      : style === "microservices"
        ? 70
        : 40;
  const scalability = clamp100(
    scalabilityBase + isolationRatio * 20 - density * 15,
  );
  ...
  const performance = clamp100(
    performanceBase - performancePenalty + m.leafNodes * 0.5,
  );
...
const crossover = (
  ...
  const style: ArchitectureStyle =
    a.benchmark.fitness >= b.benchmark.fitness ? a.style : b.style;
...
const selectSurvivors = (
  variants: ArchitectureVariant[],
  k: number,
): ArchitectureVariant[] => rankVariants(variants).slice(0, k);
...
export const evolveArchitectures = (
  ...
  for (let gen = 1; gen <= generations; gen++) {
    ...
    for (
      let i = 0;
      i < survivors.length - 1 && offspring.length < Math.floor(populationSize / 2);
      i++
    ) {
