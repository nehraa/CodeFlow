...
const node = (id: string, kind: BlueprintGraph["nodes"][0]["kind"] = "module") => ({
  id,
  name: id,
  kind,
  summary: "test node",
...
describe("genetic.ts", () => {
  describe("benchmarkVariant", () => {
    ...
    it("computes different scores for different architecture styles", () => {
      ...
      const anyDiff =
        monolith.fitness !== microservices.fitness ||
  ...
  describe("generateInitialPopulation", () => {
    ...
    it("returns variants sorted by fitness descending after ranking", () => {
      ...
      for (let i = 1; i < result.length; i++) {
        expect(result[i - 1].benchmark.fitness).toBeGreaterThanOrEqual(
          result[i].benchmark.fitness,
        );
      }
    });
