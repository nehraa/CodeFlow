// Re-export idleTraceState so callers can use it
...
const resolveFlowProvenance = (traceSpans) => {
    const counts = new Map();
    for (const span of traceSpans) {
        const provenance = span.provenance ?? "observed";
        counts.set(provenance, (counts.get(provenance) ?? 0) + 1);
    }
    let dominant = "observed";
    let dominantCount = -1;
    for (const provenance of ["observed", "simulated", "deterministic", "heuristic", "ai"]) {
 ...
 */
export const buildUserFlows = (graph, spans) => {
 ...
 */
export const computeDigitalTwinSnapshot = (graph, spans, activeWindowSecs = 60) => {
 ...
 */
export const buildSimulationSpans = (graph, nodeIds, label = "Simulated flow", runtime = "simulation") => {
 ...
 */
export const overlayActiveNodes = (graph, activeNodeIds) => {
    ...
    return {
        ...
        nodes: graph.nodes.map((node) => {
            ...
            return {
                ...node,
                traceState: {
                    ...(existing ?? { status: "idle", count: 0, errors: 0, totalDurationMs: 0, lastSpanIds: [] }),
                    // Bump the status to at least "success" so the node lights up.
                    status: existing?.status === "idle" || existing === undefined ? "success" : existing.status
                }
            };
        })
    };
