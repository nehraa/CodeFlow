import type {
  ...
  TraceSpan,
  UserFlow,
  idleTraceState as idleTraceStateFn
} from "./types.js";
 ...
 */
export const buildUserFlows = (graph: BlueprintGraph, spans: TraceSpan[]): UserFlow[] => {
 ...
 */
export const overlayActiveNodes = (graph: BlueprintGraph, activeNodeIds: string[]): BlueprintGraph => {
  ...
  return {
    ...
    nodes: graph.nodes.map((node) => {
      ...
      return {
        ...node,
        traceState: {
          ...(existing ?? { status: "idle" as const, count: 0, errors: 0, totalDurationMs: 0, lastSpanIds: [] }),
          // Bump the status to at least "success" so the node lights up.
          status:
            existing?.status === "idle" || existing === undefined ? ("success" as const) : existing.status
        }
      };
    })
  };
