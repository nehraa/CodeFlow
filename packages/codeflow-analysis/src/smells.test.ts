...
const node = (
  id: string,
  kind: BlueprintGraph["nodes"][number]["kind"] = "module",
...
describe("detectSmells", () => {
  it("detects a god-node (critical)", () => {
    const report = detectSmells(
      graph("GodNode", [
        node("god", "class", {
          responsibilities: ["r1", "r2", "r3", "r4", "r5"],
          methods: Array.from({ length: 7 }, (_, i) =>
            makeMethod(`method${i}`)
          ),
        }),
      ], [])
  ...
  it("detects orphan nodes (info)", () => {
    const report = detectSmells(
      graph("Orphan", [node("lonely", "function")], [])
    );
  ...
  it("returns health score 100 for a clean small graph", () => {
    const report = detectSmells(
      graph("Clean", [node("A", "module"), node("B", "function")], [edge("A", "B")])
    );
  ...
  it("totalSmells equals the number of individual smell records", () => {
    const report = detectSmells(
      graph("Count", [node("A", "module"), node("B", "function")], [])
    );
