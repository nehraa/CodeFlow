...
const detectHubNodes = (graph: BlueprintGraph): Smell[] => {
  ...
  return graph.nodes
    .filter(
      (n) => (inDegree.get(n.id) ?? 0) + (outDegree.get(n.id) ?? 0) >= HUB_NODE_MIN_DEGREE
    )
    .map((n) => {
      ...
      return {
        ...
        suggestion:
          "Introduce an intermediary or facade to reduce direct dependencies on this node.",
      };
    });
};
...
const detectOrphanNodes = (graph: BlueprintGraph): Smell[] => {
  ...
  return graph.nodes
    ...
    .map((n) => ({
      ...
      suggestion:
        "Verify this node is still needed; it may be dead code or missing connections.",
    }));
};
  ...
  for (const { a, b, count } of pairCounts.values()) {
    if (count >= TIGHT_COUPLING_MIN_EDGES) {
      smells.push({
        ...
        suggestion:
          "Consider merging these nodes or extracting a shared interface to reduce coupling.",
      });
    }
  }
  ...
  return graph.nodes
    ...
    .map((n) => {
      ...
      return {
        ...
        suggestion:
          "Stabilize this node by reducing its outgoing dependencies or shielding dependents with an abstraction.",
      };
    });
};
