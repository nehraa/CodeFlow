...
describe("computeGraphMetrics", () => {
  it("computes correct basic metrics for a simple graph", () => {
    const metrics = computeGraphMetrics(
      graph(
        "Simple",
        [node("A", "module"), node("B", "api"), node("C", "function")],
        [edge("A", "B"), edge("B", "C")]
      )
    );
  ...
  it("density stays <= 1 when parallel edges exist between the same pair", () => {
    const metrics = computeGraphMetrics(
      graph(
        ...
        [edge("A", "B"), { from: "A", to: "B", kind: "imports", required: false, confidence: 0.9 }]
      )
    );
  ...
  it("identifies max in-degree and max out-degree nodes", () => {
    ...
    const metrics = computeGraphMetrics(
      graph(
        "DegreeStats",
        [node("A", "module"), node("B", "module"), node("C", "module"), node("D", "module")],
        [edge("A", "B"), edge("A", "C"), edge("D", "B")]
      )
    );
  ...
  it("counts edges by kind correctly", () => {
    const metrics = computeGraphMetrics(
      graph(
        "EdgesByKind",
        [node("A", "module"), node("B", "module")],
        [edge("A", "B"), { from: "A", to: "B", kind: "imports", required: true, confidence: 1 }]
      )
    );
    ...
    const metrics = computeGraphMetrics(
      graph(
        ...
        [
          {
            ...node("A", "class"),
            contract: { ...emptyContract(), methods: [{}, {}] as any[] },
          },
          {
            ...node("B", "class"),
            contract: { ...emptyContract(), methods: [{}] as any[] },
