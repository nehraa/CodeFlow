...
async function cmdRun(opts: CliOptions) {
...
async function cmdPlan(opts: CliOptions) {
  const { createRunPlan } = await import("../plan.js");
...
async function cmdVcr(opts: CliOptions) {
...
async function cmdMermaid(opts: CliOptions) {
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg === "--graph") opts.graph = args[++i];
    else if (arg === "--target") opts.target = args[++i];
    else if (arg === "--input") opts.input = args[++i];
    else if (arg === "--output") opts.output = args[++i];
    else if (arg === "--format") opts.format = args[++i] as "json" | "mermaid";
    else if (arg === "--spans") opts.spans = args[++i];
    else if (arg === "--vcr") opts.vcr = args[++i];
    else if (arg === "--class-diagram") opts.classDiagram = true;
