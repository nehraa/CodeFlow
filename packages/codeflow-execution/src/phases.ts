...
/**
 * Returns a minimal placeholder spec draft for a node when no draft has been
 * provided. This is used as a last-resort fallback; real spec drafts are
 * typically generated by the AI agent.
 */
const placeholderSpecDraft = (node: BlueprintNode, graph: BlueprintGraph): string | undefined => {
  const ownedMethods = graph.nodes.filter(
    (c) => c.ownerId === node.id && c.kind === "function"
  );

  if (node.kind === "class") {
    const attrs = node.contract?.attributes?.length
      ? node.contract.attributes
          .map((a: { name: string; type: string }) => `  ${a.name}: ${a.type};`)
          .join("\n")
      : "  // TODO: add class attributes";
    const methods = ownedMethods.length
      ? ownedMethods
          .map((m) => {
            const inputs = m.contract?.inputs ?? [];
            const inputList = inputs
              .map((f: { name: string; type?: string }) => `${f.name}: ${f.type || "unknown"}`)
              .join(", ");
            const output = m.contract?.outputs?.[0]?.type ?? "void";
            return `  // TODO: implement ${m.name}(${inputList}): ${output}`;
          })
          .join("\n")
      : "  // TODO: add methods from blueprint contract";
  ...
  nodes: graph.nodes.map((node) =>
    isCodeBearingNode(node)
      ? {
          ...
          specDraft: node.specDraft ?? placeholderSpecDraft(node, graph) ?? undefined
        }
      : {
          ...node,
          status: node.status ?? "spec_only"
...
export const canCompleteSpecPhase = (graph: BlueprintGraph): boolean =>
  getCodeBearingNodes(graph).every((node) => Boolean(node.specDraft ?? placeholderSpecDraft(node, graph)));
