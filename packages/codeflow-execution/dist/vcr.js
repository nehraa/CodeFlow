import { idleTraceState } from "@abhinav2203/codeflow-core";
// ── Span ordering ─────────────────────────────────────────────────────────────
/**
 ...
 */
const sortSpans = (spans) => [...spans].sort((a, b) => {
...
const mergeSpanIntoState = (current, span) => {
    const nextStatus = statusPriority[span.status] > statusPriority[current.status] ? span.status : current.status;
    return {
        status: nextStatus,
 ...
 */
export const replayAtFrame = (graph, recording, frameIndex) => {
 ...
 */
export const positionToFrameIndex = (recording, position) => {
    if (recording.frames.length === 0)
        return 0;
 ...
 */
export const frameIndexToPosition = (recording, frameIndex) => {
