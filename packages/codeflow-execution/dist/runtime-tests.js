...
import { previewRuntimeValue, resolveExecutableContract, validateNodeInvocationInput, validateNodeOutput } from "./internal/runtime-contracts.js";
const parseSeedInput = (seedInput) => {
    if (!seedInput?.trim()) {
        return undefined;
    }
    try {
...
const synthesizeValue = (type, variant) => {
...
const buildInputPayload = (node, variant, seedInput) => {
    ...
    return Object.fromEntries(executableContract.inputs.map((field) => [field.name, synthesizeValue(field.type, variant)]));
};
const serializeCaseInput = (input) => {
    try {
...
const resolveTestStatus = (expectation, stepStatus) => {
    if (expectation === "pass") {
        return stepStatus;
    }
    if (expectation === "fail") {
...
export const generateRuntimeTestCases = ({ node, seedInput }) => {
    const scenarios = [
        {
            kind: "happy-path",
            expectation: "pass",
...
export const runGeneratedRuntimeTests = async ({ workspace, node, runId, testCases }) => {
    const steps = [];
    const results = [];
    for (const testCase of testCases) {
        ...
        const outputValidation = invocation.success && typeof invocation.output !== "undefined"
            ? validateNodeOutput(node, invocation.output)
            : { status: "failed", checks: [] };
        const rawStatus = inputValidation.status === "failed" || !invocation.success
            ? "failed"
            : outputValidation.status === "failed"
                ...
                : inputValidation.status === "warning" || outputValidation.status === "warning"
                    ? "warning"
                    : "passed";
        const resolvedStatus = testCase.expectation === "fail"
            ? rawStatus === "failed" || rawStatus === "warning"
                ? "passed"
                : "failed"
            : resolveTestStatus(testCase.expectation, rawStatus);
        const completedAt = new Date();
        const message = testCase.expectation === "fail"
