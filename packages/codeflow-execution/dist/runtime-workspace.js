...
import { getNodeStubPath } from "./utils.js";
import { getDefaultExecutionTarget } from "./phases.js";
import { generateRuntimeTestCases, runGeneratedRuntimeTests } from "./runtime-tests.js";
import { inferRuntimeValueType, previewRuntimeValue, resolveExecutableContract, resolveHandoffInputField, serializeRuntimeValue, summarizeExecutionStepCounts, validateEdgeHandoff, validateNodeInvocationInput, validateNodeOutput } from "./internal/runtime-contracts.js";
import { prepareRuntimeWorkspace } from "./runtime-workspace-local.js";
const SUCCESSFUL_RUNTIME_STEP_STATUSES = new Set(["passed", "warning"]);
const parseExecutionInput = (value) => {
    try {
...
const isDirectlyRunnableNode = (node) => node.kind === "function" || node.kind === "api" || node.kind === "class" || node.kind === "ui-screen";
const isPlainObject = (value) => typeof value === "object" && value !== null && !Array.isArray(value);
const sanitizeInputKey = (value) => {
...
const edgeIdFor = (edge) => `${edge.kind}:${edge.from}:${edge.to}`;
const getOwnerChildrenMap = (graph) => {
...
const hasOwnedRunnableDescendants = (nodeId, ownerChildren) => {
...
const isRunnableLeafNode = (node, ownerChildren) => isDirectlyRunnableNode(node) && !hasOwnedRunnableDescendants(node.id, ownerChildren);
const collectOwnedLeafNodes = (rootNodeId, ownerChildren) => {
    const leaves = [];
    const visit = (nodeId) => {
        const children = ownerChildren.get(nodeId) ?? [];
...
const collectAggregateNodeIds = (graph, leafNodeIds, ownerChildren) => {
...
const selectExecutionNodes = (graph, request) => {
    ...
    if (request.targetNodeId) {
        ...
        return {
            ...
            runtimeEdges: graph.edges.filter((edge) => leafNodeIds.includes(edge.from) && leafNodeIds.includes(edge.to))
        };
    }
    ...
    return {
        ...
        leafNodeIds,
        aggregateNodeIds: collectAggregateNodeIds(graph, leafNodeIds, ownerChildren).filter((nodeId) => !leafNodeIds.includes(nodeId)),
        runtimeEdges: graph.edges.filter((edge) => leafNodeIds.includes(edge.from) && leafNodeIds.includes(edge.to))
    };
};
const sortLeafNodesForExecution = (graph, leafNodeIds, edges) => {
...
const buildArtifact = ({ sourceNodeId, value, targetNodeId, edgeId, declaredType }) => ({
...
const statusMessageFromChecks = (checks, fallback) => {
    if (!checks.length) {
        return fallback;
    }
...
const createStep = ({ runId, kind, nodeId, parentNodeId, methodName, edgeId, status, startedAt, completedAt, stdout = "", stderr = "", message, blockedByStepId, inputPreview, outputPreview, artifactIds = [], contractChecks = [] }) => ({
    id: crypto.randomUUID(),
    runId,
    kind,
    nodeId,
...
const formatCompileIssue = (issue) => `${issue.line != null && issue.column != null ? `L${issue.line}:C${issue.column} ` : ""}${issue.message}`;
const buildCompileFailureResult = ({ request, selection, runId, startedAt, workspaceDir, compileDiagnostics, compileIssuesByNodeId }) => {
    const steps = [];
    const testCases = [];
    const testResults = [];
    const failingStepIds = [];
    ...
    const nodeStepsByNodeId = new Map(steps.filter((step) => step.kind === "node").map((step) => [step.nodeId, step]));
...
const buildRuntimeInputFromArtifacts = (targetNode, acceptedArtifacts) => {
    ...
    return Object.fromEntries(acceptedArtifacts.map(({ value, sourceNode, fieldName }) => [
        fieldName ?? sanitizeInputKey(sourceNode.name.split(".").pop() ?? sourceNode.name),
        value
    ]));
};
const materializeNodeOutput = (node, value) => {
    if (node.kind === "api" && isPlainObject(value) && "body" in value) {
        return value.body;
    }
    return value;
};
const createAggregateNodeStep = ({ runId, node, childSteps }) => {
    const startedAt = new Date(Math.min(...childSteps.map((step) => new Date(step.startedAt).getTime())));
    const completedAt = new Date(Math.max(...childSteps.map((step) => new Date(step.completedAt).getTime())));
...
export const runBlueprint = async (request) => {
    ...
    try {
        const compileIssuesByNodeId = new Map();
        for (const nodeId of selection.leafNodeIds) {
            const stubPath = getNodeStubPath(nodeById.get(nodeId));
            const matchedIssues = workspace.compileResult.issues.filter((issue) => stubPath && issue.filePath ? issue.filePath.endsWith(stubPath) : false);
        ...
        const orderedLeafNodeIds = sortLeafNodesForExecution(request.graph, selection.leafNodeIds, selection.runtimeEdges);
        ...
        const artifactBySourceNodeId = new Map();
        const testCases = [];
        const testResults = [];
        for (const nodeId of orderedLeafNodeIds) {
            ...
            for (const edge of incomingEdges) {
                ...
                const edgeStep = createStep({
                    ...
                    startedAt: started,
                    completedAt: completed,
                    message: statusMessageFromChecks(handoffValidation.checks, `Validated handoff from ${sourceNode.name} to ${node.name}.`),
            ...
            if (inputValidation.status === "failed") {
                ...
                const failedStep = createStep({
                    ...
                    startedAt: started,
                    completedAt: completed,
                    message: statusMessageFromChecks(inputValidation.checks, `${node.name} rejected the provided input contract.`),
                    inputPreview: previewRuntimeValue(executionInput),
            ...
            const leafStatus = !invocation.success
                ? "failed"
            ...
            if (node.kind === "class" && invocation.methodName) {
                methodStep = createStep({
                    ...
                    message: invocation.success
                        ? statusMessageFromChecks([...inputValidation.checks, ...outputValidation.checks], `${node.name}.${invocation.methodName} executed successfully.`)
            ...
            const nodeArtifact = invocation.success
                ? buildArtifact({
            ...
            const nodeStep = methodStep
                ...
                : createStep({
                    ...
                    stderr: invocation.stderr,
                    message: invocation.success
                        ? statusMessageFromChecks([...incomingChecks, ...inputValidation.checks, ...outputValidation.checks], `${node.name} executed successfully.`)
