...
export const saveBranch = async (branch) => {
    ...
    if (!normalized.name || typeof normalized.name !== "string" || normalized.name.trim().length === 0) {
        throw new Error(`saveBranch: branch.name is required and must be a non-empty string. ` +
    ...
    if (!normalized.projectName || typeof normalized.projectName !== "string" || normalized.projectName.trim().length === 0) {
        throw new Error(`saveBranch: branch.projectName is required and must be a non-empty string. ` +
