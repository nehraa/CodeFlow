...
describe("checkpoint", () => {
    ...
    describe("createCheckpointIfNeeded", () => {
        it("returns undefined when the target directory does not exist", async () => {
            await withEnv(async () => {
                const result = await createCheckpointIfNeeded("/this/path/does/not/exist", "checkpoint-1");
        ...
        it("copies the directory contents when the target has files", async () => {
            await withEnv(async () => {
                ...
                const copiedIndex = await fs.readFile(path.join(result, "src", "index.ts"), "utf8");
                expect(copiedIndex).toBe("console.log('hello')");
                const copiedPkg = await fs.readFile(path.join(result, "package.json"), "utf8");
        ...
        it("preserves directory structure in the copy", async () => {
            await withEnv(async () => {
                ...
                await fs.writeFile(path.join(targetDir, "src/lib/utils/helper.ts"), "export const helper = true");
                const result = await createCheckpointIfNeeded(targetDir, "checkpoint-nested");
                const copiedHelper = await fs.readFile(path.join(result, "src", "lib", "utils", "helper.ts"), "utf8");
        ...
        it("overwrites existing checkpoint if called again", async () => {
            await withEnv(async () => {
                ...
                const content = await fs.readFile(path.join(result2, "file.txt"), "utf8");
