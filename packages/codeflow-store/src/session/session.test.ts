...
import type { BlueprintGraph, RunPlan, PersistedSession } from "@abhinav2203/codeflow-core/schema";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const STORE_ROOT = path.join(__dirname, "../../.test-store-session");
...
const makeGraph = (overrides: Partial<BlueprintGraph> = {}): BlueprintGraph => {
  ...
  return { ...base, ...overrides } as BlueprintGraph;
};

const makeRunPlan = (): RunPlan => ({
  generatedAt: new Date().toISOString(),
  tasks: [],
  batches: [],
  warnings: []
