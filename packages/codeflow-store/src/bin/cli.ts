...
import { createApprovalRecord, getApprovalRecord, approveRecord } from "../approval/index.js";
...
const main = async () => {
  ...
  switch (`${command} ${subcommand}`) {
    ...
    case "risk assess": {
      ...
      const graph = await parseJsonFile<BlueprintGraph>(graphJson);
      const runPlan: RunPlan = {
        generatedAt: new Date().toISOString(),
        tasks: [],
        batches: [],
    ...
    case "observability merge": {
      ...
      const spans = await parseJsonFile<unknown[]>(spansJson);
