  ...
  describe("createCheckpointIfNeeded", () => {
    ...
    it("should throw clear error when checkpointId is a number", async () => {
      await withEnv(async () => {
        ...
        await expect(
          createCheckpointIfNeeded(targetDir, 42 as any)
        ).rejects.toThrow(/checkpointId must be a string/);
      });
    });
    ...
    it("should throw clear error when checkpointId is an object", async () => {
      await withEnv(async () => {
        ...
        await expect(
          createCheckpointIfNeeded(targetDir, { id: "cp-1" } as any)
        ).rejects.toThrow(/checkpointId must be a string/);
      });
    });
    ...
    it("should throw error when copying directory into itself (EINVAL)", async () => {
      await withEnv(async () => {
        ...
        await expect(
          createCheckpointIfNeeded(targetDir, "inner-checkpoint")
        ).rejects.toThrow(/cannot copy directory into itself/i);
      });
    });
  });
});
