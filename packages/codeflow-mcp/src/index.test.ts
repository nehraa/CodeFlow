...
import type { McpToolResult } from "@abhinav2203/codeflow-core/schema";

const makeFetchMock = (responseBody: unknown, ok = true, status = 200) =>
...
describe("listMcpTools", () => {
  it("returns the tools array from a valid tools/list response", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      makeFetchMock({ jsonrpc: "2.0", id: 1, result: { tools } })
    );
  ...
  it("throws when the JSON-RPC response contains an error", async () => {
    vi.stubGlobal(
      "fetch",
      makeFetchMock({ jsonrpc: "2.0", id: 1, error: { code: -32601, message: "Method not found" } })
    );
  ...
  it("returns an empty array when the result contains no tools field", async () => {
    vi.stubGlobal(
      "fetch",
      makeFetchMock({ jsonrpc: "2.0", id: 1, result: {} })
    );
...
describe("invokeMcpTool", () => {
  it("returns the content from a valid tools/call response", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      makeFetchMock({ jsonrpc: "2.0", id: 2, result: { content } })
    );
  ...
  it("throws when the JSON-RPC response contains an error", async () => {
    vi.stubGlobal(
      "fetch",
      makeFetchMock({ jsonrpc: "2.0", id: 2, error: { code: -32602, message: "Invalid params" } })
    );

    await expect(invokeMcpTool("http://mcp.example.com/mcp", "search_github", {})).rejects.toThrow(
      "Invalid params"
    );
  });
});
...
describe("extractTextFromMcpResult", () => {
  ...
  it("returns empty string when there is no text content", () => {
    const result: McpToolResult = { content: [{ type: "image" }] };
