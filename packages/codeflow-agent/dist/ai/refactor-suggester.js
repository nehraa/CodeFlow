...
const severityOf = (score) => score >= 0.7 ? "error" : score >= 0.4 ? "warning" : "info";
const id = (prefix, idx) => `${prefix}-${idx}`;
// ---------------------------------------------------------------------------
 ...
 */
const analyzeContractCompleteness = (graph) => {
    const suggestions = [];
    graph.nodes.filter(isCodeBearingNode).forEach((node, i) => {
        ...
        if (inputs.length === 0 && outputs.length === 0 && node.kind !== "module") {
            suggestions.push({
                ...
                recommendation: "Add at least one input or output field to the node's contract, or consolidate this node into its caller.",
                effort: "medium"
            });
        }
        if (inputs.length > 10) {
            suggestions.push({
                ...
                recommendation: "Consider extracting a parameter object or splitting this node into smaller nodes.",
                effort: "medium"
            });
        }
    });
 ...
 */
const analyzeOrphanNodes = (graph) => {
    const suggestions = [];
    ...
    graph.nodes.filter(isCodeBearingNode).forEach((node, i) => {
        if (!targets.has(node.id) && node.kind !== "module") {
            suggestions.push({
                ...
                recommendation: "Verify this node should have incoming edges from other nodes, or confirm it's an entry point.",
                effort: "low"
            });
        }
    });
    ...
    graph.nodes.forEach((n) => {
        ...
        if (cycleNodes.length > 0) {
            ...
            suggestions.push({
                ...
                recommendation: "Break the circular dependency by introducing an interface or consolidating nodes.",
                effort: "high"
            });
        }
    });
 ...
 */
const analyzeNodeComplexity = (graph) => {
    const suggestions = [];
    graph.nodes.filter(isCodeBearingNode).forEach((node, i) => {
        if (node.kind === "class") {
        ...
        if (node.kind === "function") {
            ...
            if (calls.length > 6) {
                suggestions.push({
                    ...
                    recommendation: "Extract groups of related calls into dedicated intermediate nodes.",
                    effort: "medium"
                });
            }
        }
 ...
 */
const analyzeDuplicateNames = (graph) => {
    const suggestions = [];
    ...
    nameCount.forEach((entries, _key) => {
        if (entries.length > 1) {
            ...
            suggestions.push({
                ...
                recommendation: "Ensure each node has a unique name. Copy-paste artifacts should be renamed or merged.",
                effort: "low"
            });
        }
    });
 ...
 */
export const analyzeAndSuggestRefactors = (graph) => {
    return [
...
const detectGlobalState = (code) => {
    const issues = [];
    const lines = code.split('\n');
    const isOutsideFunction = (lineNum, funcStart, funcEnd) => {
 ...
 */
export const suggestRefactors = (code, _nodeType) => {
    const issues = [
