...
import { analyzeAndSuggestRefactors } from './refactor-suggester.js';
...
describe('analyzeAndSuggestRefactors', () => {
    ...
    it('returns empty array when all nodes have valid contracts', () => {
        ...
        const graph = { ...emptyGraph, nodes: [node] };
    ...
    it('detects empty contract on code-bearing nodes', () => {
        ...
        const graph = { ...emptyGraph, nodes: [node] };
        const result = analyzeAndSuggestRefactors(graph);
        const emptyContractIssues = result.filter((s) => s.id.includes('empty-contract'));
    ...
    it('detects orphan nodes (no incoming edges)', () => {
        ...
        const graph = { ...emptyGraph, nodes: [orphan] };
        const result = analyzeAndSuggestRefactors(graph);
        const orphanIssues = result.filter((s) => s.id.includes('orphan'));
    ...
    it('detects duplicate node names', () => {
        ...
        const graph = { ...emptyGraph, nodes: [n1, n2] };
        const result = analyzeAndSuggestRefactors(graph);
        const dupIssues = result.filter((s) => s.id.includes('dup-name'));
    ...
    it('detects high input arity (> 10 inputs)', () => {
        ...
        const graph = { ...emptyGraph, nodes: [node] };
        const result = analyzeAndSuggestRefactors(graph);
        const arityIssues = result.filter((s) => s.id.includes('too-many-inputs'));
