...
const testBlockTitle = (node) => `${node.kind} ${node.name}`;
 ...
 */
export const generateTypeScriptTest = (node, _testStyle) => {
    const name = runtimeName(node);
    const args = inputArgs(node).join(', ');
    const callExpr = `${name}(${args})`;
    const errorTests = (node.contract?.errors ?? []).map((error) => `it('should throw or reject on ${error}', async () => {
...
// Python / pytest
// ---------------------------------------------------------------------------
const snakeCase = (name) => name.replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '');
/**
 * Generate Python/pytest test content.
 */
export const generatePythonPytest = (node, _testStyle) => {
 ...
 */
export const generateGoTest = (node, _testStyle) => {
 ...
 */
export const generateRustTest = (node, _testStyle) => {
    const name = snakeCase(runtimeName(node));
    return `#[test]
fn ${name}_test() {
    // TODO: implement test
 ...
 */
export const generateTestContent = (node, _graph, options = { language: 'typescript', framework: 'jest', testStyle: 'unit' }) => {
