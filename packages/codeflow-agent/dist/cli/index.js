...
import { executeWithContext, executeBlueprint } from '../agent/execution-context.js';
...
async function main() {
    ...
    if (options.generateBlueprint) {
        ...
        try {
            ...
            for (const node of blueprint.nodes) {
                ...
                const needsApproval = permissionManager.needsApproval(node.id, risk);
                if (needsApproval) {
                    console.log(`\n=== Approval Required ===`);
    ...
    if (options.blueprintFile) {
        ...
        const graph = JSON.parse(blueprintContent);
    ...
    if (options.mcpServerUrl)
        console.log(`MCP Server: ${options.mcpServerUrl}`);
    console.log();
    const config = {
        maxConcurrent: options.maxConcurrent,
        defaultModel: options.model
