...
import { sendToOpencodeServer, type SendToOpencodeResult } from './opencode-client.js';
 ...
 */
function parseCodeResult(content: string): CodeGenerationResult {
  try {
    ...
    return {
      ...
      notes: Array.isArray(parsed.notes) ? parsed.notes.filter((n) => typeof n === 'string') as string[] : undefined,
    };
  } catch {
 ...
 */
export async function generateNodeCode(options: GenerateCodeOptions): Promise<string> {
...
Return ONLY valid JSON with the implementation:
{
  ...
  const result = await sendToOpencodeServer(fullPrompt, { timeout }) as SendToOpencodeResult;
 ...
 */
export async function generateNodeCodeDetailed(options: GenerateCodeOptions): Promise<CodeGenerationResult> {
  const { systemPrompt, userPrompt, timeout } = options;
