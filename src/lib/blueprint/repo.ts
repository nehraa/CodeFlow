...
import { createNode, createNodeId, dedupeEdges, mergeContracts, mergeDesignCalls, toPosixPath } from "@/lib/blueprint/utils";
...
const hasJsDocSummary = (node: Node): string => {
  const maybeJsDocNode = node as Node & {
...
const buildSummary = (name: string, fallback: string): string => fallback || `Blueprint node for ${name}.`;

const getCallableSignatureNode = (declaration: Node) => {
...
const createContractFromCallable = (
  summary: string,
  declaration: Node
) => {
  ...
  const callableNode = signatureNode as Node & {
    getParameters: () => Array<{
      getName: () => string;
      getType: () => { getText: (node?: Node) => string };
    }>;
    getReturnType: () => { getText: (node?: Node) => string };
  };
...
const createMethodSpecFromDeclaration = (summary: string, declaration: Node, name: string, signature?: string) => {
...
const createClassContract = (
  summary: string,
  classDeclaration: ReturnType<ReturnType<Project["getSourceFiles"]>[number]["getClasses"]>[number]
) =>
  mergeContracts(emptyContract(), {
    ...emptyContract(),
    summary,
    responsibilities: [summary],
    ...
    methods: classDeclaration.getMethods().map((method) =>
      createMethodSpecFromDeclaration(
        buildSummary(`${classDeclaration.getName()}.${method.getName()}`, hasJsDocSummary(method)),
        method,
        method.getName(),
...
const toRelativePath = (repoPath: string, filePath: string): string =>
  toPosixPath(path.relative(repoPath, filePath));
...
const buildRoutePath = (relativePath: string): string => {
...
const buildScreenName = (relativePath: string): string => {
  const normalized = relativePath.replace(/^src\//, "");
...
const getDeclarationKey = (
  ...
  if (
    ...
    const name =
      "getName" in declaration && declaration.getName
        ? declaration.getName()
...
const collectVariableFunctions = (sourceFile: ReturnType<Project["getSourceFiles"]>[number]) =>
  sourceFile.getVariableDeclarations().filter((declaration) => {
    return Boolean(getCallableSignatureNode(declaration));
  });

export const analyzeTypeScriptRepo = async (repoPath: string): Promise<RepoGraphPart> => {
  ...
  const callableEntries: CallableEntry[] = [];
  ...
  for (const sourceFile of sourceFiles) {
    ...
    for (const classDeclaration of classes) {
      ...
      for (const method of classDeclaration.getMethods()) {
        ...
        const nodeId = createNodeId(
          "function",
          `${relativePath}:${className}.${method.getName()}`,
          `${relativePath}:${className}.${method.getName()}`
        );
    ...
    for (const functionDeclaration of sourceFile.getFunctions().filter((declaration) => declaration.getName())) {
      ...
      const kind: BlueprintNodeKind = isApiRoute ? "api" : isPageFile ? "ui-screen" : "function";
  ...
  const callableNodeIdsByClassId = new Map<string, string[]>();
  ...
  for (const entry of callableEntries) {
    ...
    for (const callExpression of (scope as Node).getDescendantsOfKind(SyntaxKind.CallExpression)) {
