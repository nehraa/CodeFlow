...
import { createNode, createNodeId, dedupeEdges, mergeContracts, mergeSourceRefs } from "@/lib/blueprint/utils";
...
const SIGNATURE_PATTERN =
  /^(?<name>[A-Za-z_$][\w$]*)\s*\((?<params>[^)]*)\)\s*(?::\s*(?<returnType>.+))?$/;
const TAGGED_ITEM_PATTERN =
  /^(screen|page|ui|api|endpoint|module|service|class|function|method)\s*:\s*(.+)$/i;

const titleToKind = (title: string): BlueprintNodeKind | null => {
...
const parseSections = (prdText: string): PrdSection[] => {
  const lines = prdText.split(/\r?\n/);
  const sections: PrdSection[] = [];
  let current: PrdSection = { title: "Overview", lines: [] };
...
const inferKindFromTaggedItem = (item: string): BlueprintNodeKind | null => {
...
const extractName = (item: string, fallbackKind: BlueprintNodeKind | null): string => {
...
const parseContractFromItem = (item: string): CodeContract => {
...
export const parsePrd = (prdText: string): PrdParseResult => {
  ...
  const upsertNode = (kind: BlueprintNodeKind, name: string, section: string, summary: string) => {
      ...
      if (workflowMatch && item.includes("->")) {
        ...
        if (steps.length >= 2) {
          ...
          for (let index = 0; index < steps.length - 1; index += 1) {
            ...
            const fromId =
              [...nodesById.values()].find((node) => node.name === fromName)?.id ??
              upsertNode("module", fromName, section.title, fromName);
            const toId =
              [...nodesById.values()].find((node) => node.name === toName)?.id ??
